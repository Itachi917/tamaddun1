...
export default function ServiceAI() {
  ...
  const [messages, setMessages] = useState([
    { role: "ai", content: `${t.aiChat.welcome} (${service})` }
  ]);
            ...
            <div className="space-y-4 pb-4">
              {messages.map((m, i) => (
                <div key={i} className={`flex gap-3 ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`p-3 rounded-lg max-w-[80%] text-start ${m.role === 'user' ? 'bg-primary text-primary-foreground' : 'bg-muted border border-border/50'}`}>
                    {m.content}
                  </div>
                </div>
              ))}
